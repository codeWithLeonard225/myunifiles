...
const CeoForm = () => {
  ...
  const validateForm = () => {
    ...
    const error = validateField("studentName", formData.studentName);
    if (error) {
      errors.studentName = error;
      valid = false;
    }
    ...
    <div className="flex flex-col items-center min-h-screen bg-gray-100 p-6">
      <div className="bg-white shadow-lg rounded-2xl p-6 w-full max-w-2xl">
        ...
        <div className="mb-6 border-t pt-4">
          <label className="block mb-1 font-semibold text-gray-700">Upload Photo *</label>
        ...
        >
          {isSubmitting ? "Saving Data..." : "Submit CEO Data"}
        </button>
      </div>

      {/* Records Table */}
      <div className="bg-white shadow-lg rounded-2xl p-6 w-full max-w-5xl mt-8">
        <h2 className="text-xl font-bold text-green-700 mb-4">Registered CEOs</h2>
          ...
          <table className="min-w-full border border-gray-300 text-sm">
            ...
            <tbody>
              {records.map((record, index) => (
                <tr key={record.id || index} className="text-center">
